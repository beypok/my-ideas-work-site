...
export class NavigationBarComponent implements OnDestroy {
   ...
   routes = [...this.defaultRoutes]
         // .filter(r => (this.isLoggedIn && r.route !== SiteRouteNames.Signin) || (!this.isLoggedIn && r.route !== SiteRouteNames.MyInfo))
   ...
   constructor(private store: Store) {
      ...
      this.isLoggedIn$.pipe(takeUntil(this.destroyed$)).subscribe(isLoggedIn => {
         ...
         this.routes = [...this.defaultRoutes].filter(r => (this.isLoggedIn && r.route !== SiteRouteNames.Signin) || (!this.isLoggedIn && r.route !== SiteRouteNames.MyInfo))
      })
   }
